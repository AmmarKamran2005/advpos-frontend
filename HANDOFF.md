...
- **Rate and margin (revised the same day).** `orders/new`: **Rate** = the product's selling price, **disabled**; **Margin (in Rs.)** and **Margin %** (a percentage *of that rate*) follow each other; the line shows **Final price: PKR X / piece** (rate + margin) and sends it as the rate. The base never moves. For the **sales role only**: margin ≤ 10% (pulled back with a red note) and never negative. `SalesController.ValidateOrderRequest` enforces the window `[SalePrice, SalePrice × 1.10]` (a paisa of slack; an item with no price is unbounded). Tested on a price of 980: 980 / 1030 / 1078 accepted, 979 / 1078.02 / 1100 refused; in the browser 10% → 98 → 1,078, Rs. 50 → 5.1% → 1,030.
