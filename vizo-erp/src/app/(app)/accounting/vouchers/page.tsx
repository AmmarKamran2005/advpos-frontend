...
// import { Plus, Wallet, Landmark, Smartphone, Banknote, ArrowDownToLine, ArrowUpFromLine, FileText } from "lucide-react";
...
//               <div className="text-2xl tabular font-bold text-brand-yellow mt-1">{formatCompact(37000)}</div>
//             </div>
...
/* GET /accounting/vouchers. `type` is "VoucherType".TypeCode; isReceipt
   says which direction the money went, so the page does not have to keep a
   list of which codes are receipts. */
type VoucherType = string;
...
export default function VouchersPage() {
  ...
  const load = React.useCallback(async () => {
    try {
      const res = await axios.get<Voucher[]>(`${API_BASE_URL}/accounting/vouchers`, {
        headers: authHeader(),
      });
      setRows(res.data);
      setError(null);
