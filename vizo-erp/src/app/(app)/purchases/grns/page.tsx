...
/* GET /purchases/grns. STOCK RISES AT THE GRN, not at the invoice.
...
/* GET /purchases/grns -> { summary, items } */
type GrnResponse = { summary: GrnSummary; items: GRN[] };
...
export default function GRNsPage() {
  ...
  /* The four cards above the table used to have their numbers typed straight
     into the markup. They come from the API now, computed across the whole
     filter rather than the rows on screen. */
  const [summary, setSummary] = React.useState<GrnSummary | null>(null);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);

  const load = React.useCallback(async () => {
    try {
      const res = await axios.get<GrnResponse>(`${API_BASE_URL}/purchases/grns`, {
        headers: authHeader(),
      });
    ...
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl tabular font-bold text-navy-900 dark:text-white mt-1">{summary?.grnsThisWeek ?? "—"}</div>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl tabular font-bold text-navy-900 dark:text-white mt-1">{summary?.poInTransit ?? "—"}</div>
            </div>
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl tabular font-bold text-success mt-1">{(summary?.unitsReceived ?? 0).toLocaleString()}</div>
        </Card>
        <Card className="p-4 bg-danger/5 border-danger/20">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl tabular font-bold text-danger mt-1">{(summary?.unitsDamaged ?? 0).toLocaleString()}</div>
            </div>
